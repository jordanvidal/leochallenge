...
// Tuto de première connexion : 5 cartes qu'on tape pour avancer, dans le
// gabarit de la story de mi-temps. Montré une fois (flag localStorage), ou
// rouvert depuis « Revoir les règles ».
//
// Il a été un écran de règles sobre jusqu'au 04/08 : une liste de lignes
// « montant → ce que ça récompense », calquée sur le mini-barème. Deux
// choses l'ont tué le même jour.
//
// La première, c'est le vocabulaire. « Journée parfaite », « 3/3 »,
// « ×1,5 », « paliers d'un même exo » : des mots qu'on ne comprend
// qu'après avoir joué une semaine, sur le premier écran, devant quelqu'un
// qui n'a rien joué du tout. Le tuto disait vrai et n'apprenait rien.
//
// La seconde, c'est la forme. Un onboarding a deux boulots — faire
// comprendre, et donner envie. Une liste fait le premier, mal, et pas le
// second. D'où le gabarit de `MiTempsScreen` : deux nappes de couleur qui
// dérivent au fond, une entrée en cascade, les zones de tap des stories.
//
// Ce que l'essai a appris, et qui explique la forme actuelle : trois
// cartes sur cinq n'ont PAS de chiffre héros. Une story raconte des
// chiffres déjà gagnés, un onboarding raconte des règles à venir — mis en
// héros, le « +20 » des 10 km passait pour la règle alors que c'est un
// exemple parmi vingt-trois. Ce qui reste du gabarit et qui travaille
// vraiment : la couleur qui change de carte en carte (la rouge du duel
// fait le travail à elle seule) et le titre énorme.
//
// Chaque carte : un kicker qui dit QUAND, un titre qui dit QUOI, un
// paragraphe qui dit COMMENT. Et on ne dit pas tout — le joker mis à part,
// le barème complet vit au Classement, à un tap, dans `MiniBareme`.
//
// Les quatre briques d'animation (nappe, CountUp, Reveal, Hero) sont
// recopiées de MiTempsScreen, qui ne les exporte pas. Les extraire dans un
// module partagé est la suite logique — mais pas avant le 07/08 : la story
// de mi-temps se révèle ce jour-là, et on ne refactore pas un écran la
// veille de sa seule sortie.
