...
import BonusPlanner from "./BonusPlanner";
...
type Props = {
  ...
  /** Catalogue + déclarations du jour. null = pas encore chargé : le
      planificateur de bonus n'existe pas plutôt qu'un écran vide. */
  ...
  /** Ouvrir directement sur le planificateur (entrée « Enchaîner »). */
...
export default function WorkoutMode({
  ...
  // Le planificateur de bonus, ouvert seulement par ses entrées dédiées
  // (« Enchaîner des bonus », la feuille de déclaration). Il n'est plus un
  // onglet en face du contrat (02/08) : le contrat ouvre la journée, le
  // planificateur est une option.
  const [planner, setPlanner] = useState(startOnBonus);
  ...
  // Le planificateur n'existe que si le catalogue est là. Sans lui, « Ma
  // séance » reste exactement l'écran qu'elle a toujours été.
  const bonusPret = bonus !== null;
