...
export type PlanBlock = BonusCatalogItem & { minutes: number };
...
function pool(
  ...
  return claimables(state)
    ...
    .map((c) => ({ ...c, minutes: bonusMinutes(c) }));
}
