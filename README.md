...
3. **Crons GitHub Actions** (`.github/workflows/*.yml`) : tout le reste passe par là (secret `CRON_SECRET` en secret de repo). `daily-event` (tirage du jour, 9h Paris), `streak-risk` (série en danger, 17h), `last-standing` (dernier debout, 21h30), `weekly-recap` (récap du lundi, 10h) et `announce-duels` (annonce des duels, dimanche 19h). ⚠️ Ces workflows ne tapent que l'instance d'origine — pour une nouvelle bande, dupliquer les appels (cf. `supabase/README-nouvelle-instance.md`).
