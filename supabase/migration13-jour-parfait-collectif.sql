-- =============================================================
-- Migration 13 : le jour parfait collectif.
--
-- Toute la bande à 3/3 le même jour → +5 pts pour chacun. Un bonus
-- égal pour tous ne change RIEN au classement relatif : sa valeur est
-- purement collective — le retardataire de 22h50 ne défend plus que sa
-- série, il porte celle du groupe.
--
...
-- pour inscrire 'co_lead' dans la contrainte, utilisé par /api/moments.
-- =============================================================

insert into public.bonus_catalog (key, kind, emoji, label, points, sort) values
  ('jour_parfait_collectif', 'execution', '🤝', 'Jour parfait collectif : toute la bande à 3/3', 5, 16)
...
-- -------------------------------------------------------------
-- Vue daily_points : identique à la migration 11, plus le bonus
-- collectif dans execution_bonus. Colonnes inchangées → replace.
-- -------------------------------------------------------------
...
-- rendrait pas compte des 5 pts (et quitte_ou_double ne les
