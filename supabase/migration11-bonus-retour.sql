-- =============================================================
-- Migration 11 : le retour.
--
-- 3/3 le lendemain d'un jour à zéro = 3 pts. Ça vise le mec qui a
-- décroché et qui hésite à revenir, pas celui qui est déjà devant.
--
-- Pourquoi ce n'est pas farmable : casser une série à ×2 coûte 5 pts
-- le jour sauté, puis ~5 pts/jour pendant les 6 jours qu'il faut pour
-- remonter le multiplicateur. Sauter un jour pour toucher 3 pts est
-- toujours perdant, quel que soit le profil. C'est le barème qui le
-- garantit, pas la bonne foi des joueurs.
--
-- Deux gardes contre les faux retours :
--   1. hier doit être un VRAI zéro (aucune entrée, ou entrée à 0 exo) ;
--   2. le joueur doit avoir été dans la partie AVANT hier — sinon le
--      1er jour du challenge et le 1er jour d'un nouveau venu
--      déclencheraient un « retour » d'un néant.
--
-- Additive : aucun point déjà acquis ne bouge (personne n'a encore de
-- jour à zéro suivi d'un 3/3).
-- =============================================================

insert into public.bonus_catalog (key, kind, emoji, label, points, sort) values
  ('retour', 'execution', '🔙', 'Le retour : 3/3 après un jour à zéro', 3, 15)
on conflict (key) do nothing;

-- -------------------------------------------------------------
-- Vue daily_points : identique à l'existant, plus `comeback` dans
-- execution_bonus. Colonnes inchangées, donc replace suffit et
-- player_badges continue de marcher.
-- -------------------------------------------------------------
...
select
  ...
  pm.base_pts + pm.execution_bonus + pm.event_bonus + pm.claim_bonus + pm.quitte_bonus
    + case when mw.player_id is not null
           then public.bonus_value('jour_miroir') else 0 end as points,
  pm.base_pts as base_points,
  pm.execution_bonus + pm.event_bonus + pm.claim_bonus + pm.quitte_bonus
    + case when mw.player_id is not null
           then public.bonus_value('jour_miroir') else 0 end as bonus_points
from premirror pm
...
-- rendrait pas compte des 3 pts (et quitte_ou_double ne les
-- doublerait pas comme le fait la vue).
-- -------------------------------------------------------------
...
as $$
  ...
  premirror as (
    select
      ...
      b_retour, b_pompes_double, b_happy_hour, b_leve_tot, claim_bonus,
      case when event_key = 'quitte_ou_double' and perfect
           then (exos + case when perfect then 2 else 0 end) * multiplier
                + b_premier_du_jour + b_avant_8h + b_apres_22h + b_seance_20min
                + b_seance_rapide + b_retour + b_pompes_double + b_happy_hour + b_leve_tot
                + claim_bonus
