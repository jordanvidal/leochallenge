-- =============================================================
-- Migration 18 : le trio matinal ne s'empile plus.
--
-- Constat S1 : premier du jour, avant 8h et séance la plus rapide
-- récompensent le MÊME comportement — s'entraîner tôt et vite. Ils
-- tombaient donc tous sur le même joueur : 30 des 44 pts du trio
-- pour une seule personne, jusqu'à +11/jour sans concurrence
-- possible pour ceux qui s'entraînent le soir. On payait trois fois
-- la même chose. Empilement max après correction : +5/jour.
--
-- Trois corrections, DATÉES au 20/07 (la S1 reste calculée aux
-- règles de l'époque) :
--   1. le trophée « premier du jour » TOURNE : si tu as été premier
--      à finir hier, le +3 du jour va au premier des autres.
--      L'exclusion ne dure qu'un jour ; si le tenant est le seul à
--      finir, le trophée n'est pas attribué ce jour-là.
--   2. avant 8h ne se cumule plus avec premier du jour le même jour.
--   3. seance_rapide passe de 5 à 2 au catalogue (5 figé pour la S1).
--
-- daily_points et player_breakdown sont recopiés de la migration 17,
-- seuls ces trois points changent. Le `with` de tête devient
-- `with recursive` : la rotation se calcule en chaîne depuis le
-- 20/07 (le gagnant de la veille est porté par la récursion).
-- =============================================================

update public.bonus_catalog set points = 2 where key = 'seance_rapide';

-- Le label du catalogue s'affiche dans le détail des points ; la
-- migration 15 a changé la règle du quitte ou double sans le mettre
-- à jour. Déjà appliqué en prod à la main — archivé ici.
update public.bonus_catalog
   set label = 'Quitte ou double : ta base du jour ×2 si 3/3'
 where key = 'quitte_ou_double';
