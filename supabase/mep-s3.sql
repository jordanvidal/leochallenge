-- =============================================================
-- MEP SAISON 3 — un seul passage, à coller dans l'éditeur SQL
-- Supabase. Assemblé mécaniquement depuis migration29 et
-- migration31 : aucune ligne de SQL n'a été recopiée à la main.
--
-- CE QUE ÇA FAIT, MAINTENANT (invisible pour le groupe) :
--   · la vue daily_points et le RPC player_breakdown de la S3.
--     Leurs 34 bornes de date font que S1 et S2 gardent leur
--     barème et que la S3 s'allume seule à minuit ;
--   · la roue de tirage S3 (le tirage du 26/07 est déjà figé en
--     base, elle ne peut donc rien changer aujourd'hui) ;
--   · semaine_pleine / abdos_double / squats_double : ni
--     'exercise' ni déclarables, donc absents de la feuille ;
--   · la colonne family, vide — sans valeur, claimableGroups()
--     retombe sur la liste à plat d'avant.
--
-- CE QUE ÇA REPORTE À 00h05 (les seules puces visibles) :
--   le 10 km, les six bonus de cardio, et le rangement par zone.
--   Un job pg_cron unique s'en charge, puis se désinscrit.
--
-- SÉCURITÉ : tout est dans UNE transaction. Le contrôle de
-- non-régression (protocole §5 de la migration 29) lève une
-- exception si un seul point de S1 ou S2 bouge — et l'exception
-- annule l'intégralité de ce fichier, job pg_cron compris.
-- =============================================================
...
-- -------------------------------------------------------------
-- Neutralisation de la seule partie visible de la 29 : le 10 km
-- est une puce 'exercise', il apparaîtrait dans la feuille avant
-- l'heure. On le retire et on rend son cran d'ordre. Le job de
-- minuit le remet — l'insert de la 29 est gardé par un
-- `not exists`, il est donc rejouable tel quel.
-- -------------------------------------------------------------

delete from public.bonus_catalog where key = 'course_10km';
update public.bonus_catalog
   set sort = sort - 1
 where kind = 'exercise' and sort >= 9;

-- -------------------------------------------------------------
-- La colonne family, vide. Invisible : claimableGroups() ne
-- range que si au moins une ligne a une famille.
-- -------------------------------------------------------------
...
-- -------------------------------------------------------------
-- Contrôle de non-régression — protocole §5 de la migration 29.
-- Les deux comptes doivent être nuls. Sinon : exception, et la
...
declare
  ...
  n_apparues int;
begin
  select count(*) into n_bougent
  from public.daily_points nv
  join avant a using (player_id, day)
  where nv.points       is distinct from a.points
     or nv.bonus_points is distinct from a.bonus_points
     or nv.streak_pos   is distinct from a.streak_pos;

  select count(*) into n_apparues
  from public.daily_points nv
  full join avant a using (player_id, day)
  where nv.player_id is null or a.player_id is null;

  if n_bougent <> 0 or n_apparues <> 0 then
    raise exception
      'REGRESSION S1/S2 : % jours dont les points bougent, % lignes apparues ou disparues. TOUT EST ANNULE.',
      n_bougent, n_apparues;
  end if;

  raise notice 'Bloc A applique : 0 point deplace sur S1 et S2.';
end
$VERIF$;
...
begin
  if (now() at time zone 'Europe/Paris')::date < date '2026-07-27' then
    raise exception 'MEP S3 : declenchement trop tot (%), rien applique.',
...
select cron.unschedule('mep-s3-bloc-b');
$MEPS3$);
