-- =============================================================
-- Migration 28 — le « premier du jour » s'annonce dans le fil
-- =============================================================
-- Le bonus automatique « 🌅 premier du jour » (+3, rotation du trophée
-- depuis le 20/07) est bien calculé et crédité, mais il ne se voyait
-- que dans le détail des points. Contrairement au joker, aux badges ou
-- au jour parfait collectif, aucune carte ne l'annonçait — celui qui
-- finit premier avait l'impression de ne rien gagner.
--
-- Deux ajouts, tous les deux sûrs et sans effet sur les scores :
--
-- 1) `daily_points` expose une colonne `premier_du_jour` (booléen). La
--    vue calculait déjà le gagnant (CTE first_done, rotation comprise) :
--    on ne fait que le SURFACER, aucun point ne change, aucune ligne
--    n'apparaît ou ne disparaît. La colonne est ajoutée EN FIN de liste
--    (après jokered) — seule forme qu'accepte `create or replace view`.
--    Ce fichier recopie la vue de la migration 27 (fix joker inclus) à
--    l'identique, colonne en plus.
--
-- 2) On élargit le CHECK de `feed_events.kind` d'une valeur `premier`,
--    comme la migration 25 l'a fait pour `joker`. Élargir une contrainte
--    ne peut invalider aucune ligne existante : sûr et rejouable.
--
-- La carte du fil est posée par /api/moments (jour + gagnant de la
-- veille, dédup par jour) et volontairement SANS push : un « premier du
-- jour » quotidien poussé à six personnes serait le bruit que le produit
-- refuse. Elle se découvre en ouvrant le fil, comme le reste.
-- =============================================================
...
base as (
  select
    ...
    -- Le gagnant « premier du jour » du jour (rotation comprise), exposé
    -- pour le fil : c'est fd qui porte déjà la vérité, on ne recalcule rien.
...
select
  ...
  pm.jokered,
  -- premier_du_jour EN DERNIER : « create or replace view » n'ajoute
  -- qu'en fin de liste (42P16). jokered reste l'avant-dernière colonne.
  pm.premier_du_jour
from premirror pm
...
-- feed_events.kind : une valeur de plus, comme la migration 25 pour joker.
