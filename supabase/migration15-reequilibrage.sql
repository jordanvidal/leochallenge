-- =============================================================
-- Migration 15 : rééquilibrage des bonus (S2, à partir du 20/07).
--
-- Constat S1 : 66 % des points venaient des bonus. Deux coupables :
-- quitte ou double (112 pts distribués en UN jour, 21 % de tous les
-- points du jeu) et la séance éclair (+5 quasi quotidien, un second
-- salaire). La base — cocher ses exos — ne pesait plus qu'un tiers.
--
-- Deux changements, DATÉS au lundi 20/07 pour ne pas réécrire
-- l'historique (tout le scoring est recalculé à la volée — sans
-- garde-date, changer une valeur changerait la S1 déjà archivée) :
--   1. quitte ou double ne double plus que les points de BASE du
--      jour (plafond de fait ~+10). Avant le 20/07 : ancien calcul.
--   2. seance_20min passe de 5 à 2 au catalogue ; les jours d'avant
--      gardent le 5 en dur dans les deux définitions.
--
-- daily_points et player_breakdown sont recopiés de la migration 14,
-- seuls les deux points ci-dessus changent.
-- =============================================================

update public.bonus_catalog set points = 2 where key = 'seance_20min';
