...
describe("les jours de doublement", () => {
  ...
  it("compte le double sur la puce tirée", () => {
    ...
    const plan = composePlan(state, MOI, {
      budget: null,
      goal: null,
      zones: new Set<BonusFamily>(["cardio"]),
    })!;
  ...
  it("atteint un objectif plus vite quand la puce tirée est dedans", () => {
    const zones = new Set<BonusFamily>(["cardio"]);
    const normal = shortestTimeFor(etat(), MOI, 10, zones);
    const double = shortestTimeFor(etatDouble("corde_10min"), MOI, 10, zones);
    expect(normal).not.toBeNull();
    expect(double).not.toBeNull();
    // Doubler une puce ne peut jamais rallonger le chemin vers l'objectif.
    expect(double!).toBeLessThanOrEqual(normal!);
  });
});
